...
28. **`register` didn't actually log the connection in, unlike `login`**: `handle_login` has always set `ctx.username`/`ctx.rating` on success, but `handle_register` just forwarded `auth_register`'s result without touching `ctx` at all — so a freshly-registered user landed on `HomeScreen` (the client already treated any `success: True` auth reply the same way, login or register) but every subsequent server-side action that reads `ctx.username` saw `None`: `handle_play` explicitly rejects with `"must be logged in to play"`, and room creation/join built a `RoomParticipant` with a `None` username/rating. Required a disconnect + explicit login to actually unlock anything. Fixed at the source: `auth.register()` now returns `"rating": DEFAULT_RATING` (`server/config.py`, 1200) alongside `success`, and `handle_register` sets `ctx.username`/`ctx.rating` from that response exactly like `handle_login` does — no client-side change needed, since `LoginScreen` was already reply-shape-agnostic.
