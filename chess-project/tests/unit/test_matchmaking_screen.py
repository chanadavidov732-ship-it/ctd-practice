...
def _make_screen(engine_new_ratings: dict) -> tuple[MatchmakingScreen, FakeBridge]:
    bridge = FakeBridge(FakeEngine(engine_new_ratings))
    screen = MatchmakingScreen(bridge)
...
def test_enter_game_keeps_prior_rating_when_username_absent_from_new_ratings(monkeypatch):
    screen, bridge = _make_screen({})
    monkeypatch.setattr(game_runner_module, "run_graphical_game", lambda bridge, engine: True)

    screen._enter_game({"your_color": "w"})

    assert screen.rating == 1200
